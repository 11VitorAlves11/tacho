...
- [x] **Importação inteligente via Gemini** — `app/gemini.py`
      (`google-genai==2.17.0`, `Settings.gemini_api_key` opcional —
      `is_available()` false sem chave, a app funciona na mesma). Output
      ...
      - **(a) fallback de extração** — `app/tasks.py::
        ...
        normal fica bit-a-bit igual ao de antes (`pingodoce.pt` real, 10
        ingredientes, como sempre).
      - **(b) importação por foto** — `POST /recipes/import/photo`
        (síncrono, 1-3 fotos, devolve um rascunho para revisão, nunca
        grava direto). `RecipeForm.tsx` ganhou `RecipeFormInitial` (tipo
        mais fraco do que `Recipe`) para pré-preencher a partir do
        rascunho sem inventar id/is_favorite/etc.; `AddRecipe.tsx` ganhou
        a aba "Por foto".
      - **2026-08-11, chave real recebida e testada** (`GEMINI_API_KEY`,
        já em produção — ver TODO.md entrada de deploy mais abaixo):
        `_MODEL` corrigido de `gemini-2.5-flash` (deixou de estar
        disponível para novas chaves — erro 404 real ao testar) para
        `gemini-flash-latest`, confirmado por `client.models.list()`
        contra a conta real. **Testado com a API real**:
        `extract_from_html` com HTML de receita fabricado (título,
        porções, tempos, 4 ingredientes com quantidade/unidade, 3 passos
        com duração — tudo extraído corretamente); `extract_from_images`
        com uma foto real (do prato acabado, não uma página de livro — a
        única disponível nesta sessão) devolveu só o título inferido da
        imagem e ingredientes/passos **vazios**, em vez de inventados —
        confirma que o modelo não alucina quando não há texto de receita
        visível, o comportamento conservador pretendido; endpoint
        `POST /recipes/import/photo` testado via curl com a chave real,
        200 OK. **Ainda por validar** (não havia uma foto de página de
        livro/receita manuscrita disponível nesta sessão): a qualidade da
        extração a partir de fotos de texto real — validar cedo com um
        caso desses assim que surgir, o aviso do PRD sobre o LLM poder
        "corrigir" quantidades silenciosamente mantém-se.
