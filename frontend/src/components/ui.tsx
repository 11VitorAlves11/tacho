...
export function PasswordInput({ className = '', ...props }: Omit<InputHTMLAttributes<HTMLInputElement>, 'type'>) {
  ...
  return <div className={`relative ${className}`}><input type={visible ? 'text' : 'password'} className="min-h-11 w-full rounded-xl border border-border bg-surface py-2 pl-3 pr-12 text-sm text-text-primary outline-none transition focus:border-primary-forest focus:ring-2 focus:ring-primary-forest/15" {...props} /><button type="button" onClick={() => setVisible((value) => !value)} aria-label={visible ? t('Ocultar password') : t('Mostrar password')} aria-pressed={visible} className="absolute inset-y-0 right-0 flex w-11 items-center justify-center rounded-r-xl text-text-secondary transition-colors hover:text-text-primary"><span className="flex size-5 items-center justify-center">{visible ? <EyeOffIcon className="block size-5" /> : <EyeIcon className="block size-5" />}</span></button></div>
}
...
export function BottomSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-labelledby="sheet-title">
      <button className="absolute inset-0 bg-black/35" onClick={onClose} aria-label={t('Fechar')} />
      <section className="absolute inset-x-0 bottom-0 max-h-[85svh] overflow-y-auto rounded-t-3xl border-t border-border bg-surface px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-3 shadow-2xl">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <button onClick={onClose} className="min-h-11 px-2 text-sm font-semibold text-forest-text">{t('Fechar')}</button>
        </div>
...
export function Modal({ open, title, onClose, children }: { open: boolean; title: string; onClose: () => void; children: ReactNode }) {
  if (!open) return null
  return <div className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="modal-title"><button className="absolute inset-0 bg-black/40" onClick={onClose} aria-label={t('Fechar')} /><section className="relative w-full rounded-t-3xl border border-border bg-surface p-5 shadow-2xl sm:max-w-md sm:rounded-2xl"><div className="flex items-center justify-between gap-3"><h2 id="modal-title" className="text-lg font-bold text-text-primary">{title}</h2><button onClick={onClose} className="min-h-11 px-2 text-sm font-semibold text-forest-text">{t('Fechar')}</button></div><div className="mt-3">{children}</div></section></div>
}

export function ConfirmDialog({ open, title, description, confirmLabel = t('Confirmar'), onCancel, onConfirm }: { open: boolean; title: string; description: string; confirmLabel?: string; onCancel: () => void; onConfirm: () => void }) {
  return <Modal open={open} title={title} onClose={onCancel}><p className="text-sm leading-relaxed text-text-secondary">{description}</p><div className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={onCancel}>{t('Cancelar')}</Button><Button variant="danger" onClick={onConfirm}>{confirmLabel}</Button></div></Modal>
}

export function Toast({ children, onDismiss }: { children: ReactNode; onDismiss?: () => void }) {
  return <div role="status" className="fixed inset-x-4 bottom-28 z-[60] mx-auto flex max-w-sm items-center gap-3 rounded-xl border border-border bg-surface p-4 text-sm font-medium text-text-primary shadow-xl lg:bottom-6">{children}{onDismiss && <button onClick={onDismiss} className="ml-auto text-xs font-semibold text-forest-text">{t('Fechar')}</button>}</div>
}
