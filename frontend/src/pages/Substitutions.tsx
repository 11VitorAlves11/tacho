...
export function Substitutions() {
  ...
  useEffect(() => { listSubstitutions().then(setItems).catch(() => setError(t('Não foi possível carregar as substituições.'))) }, [])
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setItems((current) => [...current.filter((item) => item.id !== saved.id), saved].sort((a, b) => a.ingredient_name.localeCompare(b.ingredient_name, getLanguage())))
      setForm(emptyForm); setEditingId(undefined)
    } catch { setError(t('Não foi possível guardar a substituição.')) } finally { setBusy(false) }
  }
  ...
  return <PageShell><h1 className="text-2xl font-bold text-text-primary sm:text-3xl">{t('Substituições de ingredientes')}</h1><p className="mt-1 text-sm text-text-secondary">{t('Alternativas conhecidas aparecem automaticamente quando faltar um ingrediente.')}</p>{error && <p role="alert" className="mt-4 text-sm text-danger">{error}</p>}<div className="mt-6 grid gap-5 lg:grid-cols-[1fr_380px]"><div className="space-y-3">{items.map((item) => <article key={item.id} className="rounded-2xl border border-border bg-surface p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-text-primary">{item.ingredient_name} <span className="text-text-secondary">→</span> {item.substitute_name}</p><p className={`mt-1 text-xs font-semibold ${item.is_verified ? 'text-forest-text' : 'text-accent-orange'}`}>{item.is_verified ? t('Alternativa verificada') : t('Sugestão por verificar')}</p></div><div className="flex"><button type="button" onClick={() => edit(item)} className="rounded-lg px-2 text-xs font-semibold text-forest-text">{t('Editar')}</button><button type="button" onClick={() => remove(item.id)} aria-label={t('Apagar substituição')} className="flex size-8 items-center justify-center text-text-secondary"><XIcon className="size-4" /></button></div></div>{item.quantity_ratio && <p className="mt-2 text-sm text-text-secondary">{t('Usar')} {item.quantity_ratio}{t('× a quantidade original.')}</p>}{item.note && <p className="mt-1 text-sm text-text-secondary">{item.note}</p>}</article>)}</div><form onSubmit={submit} className="h-fit rounded-2xl border border-border bg-surface p-5"><h2 className="font-bold text-text-primary">{editingId ? t('Editar alternativa') : t('Nova alternativa')}</h2><Field label={t('Ingrediente original')} value={form.ingredient_name} onChange={(value) => setForm({ ...form, ingredient_name: value })} required /><Field label={t('Substituto')} value={form.substitute_name} onChange={(value) => setForm({ ...form, substitute_name: value })} required /><Field label={t('Rácio de quantidade')} value={form.quantity_ratio} onChange={(value) => setForm({ ...form, quantity_ratio: value })} type="number" placeholder={t('ex.: 0.5')} /><Field label={t('Notas de utilização')} value={form.note} onChange={(value) => setForm({ ...form, note: value })} /><label className="mt-4 flex items-center gap-2 text-sm font-semibold text-text-primary"><input type="checkbox" checked={form.is_verified} onChange={(event) => setForm({ ...form, is_verified: event.target.checked })} className="size-4 accent-primary-forest" />{t('Alternativa verificada')}</label><div className="mt-5 flex gap-2"><button disabled={busy} className="min-h-11 flex-1 rounded-xl bg-primary-forest px-4 text-sm font-semibold text-white disabled:opacity-50">{busy ? t('A guardar…') : t('Guardar')}</button>{editingId && <button type="button" onClick={() => { setEditingId(undefined); setForm(emptyForm) }} className="rounded-xl border border-border px-3 text-sm font-semibold">{t('Cancelar')}</button>}</div></form></div></PageShell>
}
