...
export function MealPlan() {
  ...
  async function copyPreviousWeek() {
    ...
    try {
      ...
      setPlanFeedback(t('Semana anterior copiada. As refeições existentes foram preservadas.'))
    } catch {
      setPlanFeedback(t('Não foi possível copiar a semana anterior.'))
    } finally {
  ...
  async function saveCurrentWeekAsTemplate() {
    const name = window.prompt(t('Nome do modelo semanal:'))?.trim()
    if (!name) return
    setPlanActionBusy(true)
    try {
      const template = await saveMealPlanTemplate(name, toDateKey(weekStart))
      setTemplates((current) => [...current.filter((item) => item.id !== template.id), template].sort((a, b) => a.name.localeCompare(b.name, getLanguage())))
      setSelectedTemplateId(template.id)
      setPlanFeedback(t('Modelo “{name}” guardado.', { name: template.name }))
    } catch {
      setPlanFeedback(t('Não foi possível guardar o modelo.'))
    } finally {
      setPlanActionBusy(false)
    }
  }
  ...
  async function applySelectedTemplate() {
    ...
    try {
      ...
      setPlanFeedback(t('Modelo aplicado. As refeições existentes foram preservadas.'))
    } catch {
      setPlanFeedback(t('Não foi possível aplicar o modelo.'))
    } finally {
      setPlanActionBusy(false)
    }
  }
  ...
  async function previewSuggestions() {
    ...
    try {
      ...
      setPlanFeedback(t('Não foi possível criar uma sugestão para esta semana.'))
    } finally {
  ...
  async function applySuggestions() {
    ...
    try {
      ...
      setPlanFeedback(t('{count} sugestões adicionadas ao planeamento.', { count: suggestions.length }))
    } catch {
      setPlanFeedback(t('A sugestão foi aplicada apenas parcialmente. Confirma os espaços antes de tentar novamente.'))
      setSuggestionOpen(false)
    } finally {
      setSuggestionBusy(false)
    }
  ...
  return <PageShell wide>
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div><h1 className="text-2xl font-bold tracking-tight text-text-primary sm:text-3xl">{t('Planeamento de Refeições')}</h1><p className="mt-1 text-sm text-text-secondary">{t('Organiza todas as refeições da semana.')}</p></div>
    ...
    {error && <p role="alert" className="mt-5 rounded-xl border border-danger/20 bg-danger/5 p-4 text-sm text-text-secondary">{t('Não foi possível carregar o planeamento.')}</p>}
    <div className="mt-5 flex flex-wrap items-end gap-2 rounded-2xl border border-border bg-surface p-3"><button type="button" disabled={planActionBusy || suggestionBusy} onClick={previewSuggestions} className="min-h-10 rounded-xl bg-primary-forest px-4 text-sm font-semibold text-white disabled:opacity-50">{suggestionBusy ? t('A preparar…') : t('Sugerir semana')}</button><button type="button" disabled={planActionBusy} onClick={copyPreviousWeek} className="min-h-10 rounded-xl border border-border px-3 text-sm font-semibold text-forest-text disabled:opacity-50">{t('Copiar semana anterior')}</button><button type="button" disabled={planActionBusy} onClick={saveCurrentWeekAsTemplate} className="min-h-10 rounded-xl border border-border px-3 text-sm font-semibold text-forest-text disabled:opacity-50">{t('Guardar como modelo')}</button><label className="min-w-48 flex-1 text-xs font-semibold text-text-secondary">{t('Modelo semanal')}<select value={selectedTemplateId} onChange={(event) => setSelectedTemplateId(event.target.value)} className="mt-1 min-h-10 w-full rounded-xl border border-border bg-surface px-3 text-sm text-text-primary"><option value="">{t('Escolher modelo…')}</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.name} ({template.slots.length} {t('refeições)')}</option>)}</select></label><button type="button" disabled={!selectedTemplateId || planActionBusy} onClick={applySelectedTemplate} className="min-h-10 rounded-xl bg-primary-forest px-4 text-sm font-semibold text-white disabled:opacity-50">{t('Aplicar')}</button></div>
    ...
    <div className="mt-6 space-y-6 lg:hidden">
      {days.map((day, dayIndex) => {
        ...
        return <section key={dayKey} className={`rounded-2xl border p-3 ${isToday ? 'border-primary-forest bg-primary-soft/40' : 'border-border bg-muted/30'}`}><h2 className="mb-3 flex items-baseline justify-between gap-3 px-1 text-lg font-bold text-text-primary"><span>{weekdayLabel(dayIndex)}</span><span className={`text-sm font-semibold ${isToday ? 'text-forest-text' : 'text-text-secondary'}`}>{isToday ? t('Hoje ·') : ''}{formatDayShort(day)}</span></h2><div className="space-y-3">{MEALS.map((meal) => <MobileMeal key={meal.key} label={t(meal.label)} entry={entryFor(dayKey, meal.key)} recipes={recipes} days={days} onAssign={(id) => assign(dayKey, meal.key, id)} onRepeat={(targetDay, targetMeal, recipeId) => assign(targetDay, targetMeal, recipeId)} onRemove={() => remove(dayKey, meal.key)} />)}</div></section>
      })}
    </div>
    ...
    <div className="mt-7 hidden overflow-hidden rounded-2xl border border-border bg-surface lg:block">
      ...
      {MEALS.map((meal) => <div key={meal.key} className="grid min-h-36 grid-cols-[120px_repeat(7,minmax(110px,1fr))] border-b border-border last:border-0"><div className="p-4 text-sm font-bold text-text-primary">{t(meal.label)}</div>{days.map((day) => { const dayKey = toDateKey(day); return <DesktopMeal key={dayKey} entry={entryFor(dayKey, meal.key)} recipes={recipes} days={days} isDragging={draggedMeal !== null} onAssign={(id) => assign(dayKey, meal.key, id)} onRepeat={(targetDay, targetMeal, recipeId) => assign(targetDay, targetMeal, recipeId)} onRemove={() => remove(dayKey, meal.key)} onDragStart={(recipeId) => setDraggedMeal({ day: dayKey, meal: meal.key, recipeId })} onDragEnd={() => setDraggedMeal(null)} onDrop={() => moveMeal(dayKey, meal.key)} /> })}</div>)}
    </div>
    <Modal open={suggestionOpen} title={t('Sugestão para a semana')} onClose={() => !suggestionBusy && setSuggestionOpen(false)}>
      <p className="text-sm text-text-secondary">{t('Revê a proposta antes de a aplicar. Podes retirar refeições; os espaços já ocupados não são alterados.')}</p>
...
function RecipePicker({ recipes, onAssign, variant = 'add' }: { recipes: RecipeSummary[]; onAssign: (id: string) => void; variant?: 'add' | 'replace' }) {
  ...
  const filteredRecipes = recipes.filter((recipe) => recipe.title.toLocaleLowerCase(getLanguage()).includes(query.trim().toLocaleLowerCase(getLanguage())))
...
function RepeatMeal({ recipe, days, onRepeat }: { recipe: RecipeSummary; days: Date[]; onRepeat: (day: string, meal: MealType, recipeId: string) => Promise<void> }) {
  ...
  return <><button type="button" onClick={() => setOpen(true)} className="rounded-lg bg-white/90 px-2.5 py-1.5 text-xs font-semibold text-neutral-700 shadow-sm hover:bg-white">{t('Repetir')}</button><Modal open={open} title={t('Repetir {name}', { name: recipe.title })} onClose={() => setOpen(false)}><label className="block text-sm font-medium text-text-primary">{t('Destino')}<select value={target} onChange={(event) => setTarget(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-border bg-surface px-3 text-text-primary outline-none focus:border-primary-forest">{days.flatMap((day, dayIndex) => MEALS.map((meal) => <option key={`${toDateKey(day)}-${meal.key}`} value={`${toDateKey(day)}|${meal.key}`}>{weekdayLabel(dayIndex)}, {formatDayShort(day)} · {t(meal.label)}</option>))}</select></label><label className="mt-4 flex items-center gap-2 text-sm font-medium text-text-primary"><input type="checkbox" checked={recurring} onChange={(event) => setRecurring(event.target.checked)} className="size-4 accent-primary-forest" />{t('Repetir automaticamente')}</label>{recurring && <div className="mt-3 grid grid-cols-2 gap-2"><label className="text-xs font-semibold text-text-secondary">{t('Intervalo')}<select value={intervalWeeks} onChange={(event) => setIntervalWeeks(Number(event.target.value))} className="mt-1 min-h-11 w-full rounded-xl border border-border bg-surface px-2 text-sm text-text-primary"><option value="1">{t('Todas as semanas')}</option><option value="2">{t('A cada 2 semanas')}</option><option value="4">{t('A cada 4 semanas')}</option></select></label><label className="text-xs font-semibold text-text-secondary">{t('Termina em')}<input type="date" value={endsOn} onChange={(event) => setEndsOn(event.target.value)} className="mt-1 min-h-11 w-full rounded-xl border border-border bg-surface px-2 text-sm text-text-primary" /></label></div>}{repeatError && <p role="alert" className="mt-3 text-sm text-danger">{t('Não foi possível repetir esta refeição.')}</p>}<button type="button" disabled={saving} onClick={repeat} className="mt-5 min-h-11 w-full rounded-xl bg-primary-forest px-4 text-sm font-semibold text-white disabled:opacity-60">{saving ? t('A repetir…') : recurring ? t('Criar repetição') : t('Repetir refeição')}</button></Modal></>
}
