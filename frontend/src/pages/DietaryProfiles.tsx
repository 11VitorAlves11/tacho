...
import type { DietaryProfile } from '../api/types'
import { PageShell } from '../components/PageShell'
import { PlusIcon, XIcon } from '../components/icons'

const emptyForm = { name: '', allergies: '', intolerances: '', preferences: '' }
...
export function DietaryProfiles() {
  ...
  useEffect(() => { listDietaryProfiles().then(setProfiles).catch(() => setError('Não foi possível carregar os perfis.')) }, [])
  ...
  async function submit(event: FormEvent) {
    ...
    setError('')
    try {
      const saved = await saveDietaryProfile({ name: form.name, user_id: null, allergies: splitTerms(form.allergies), intolerances: splitTerms(form.intolerances), preferences: splitTerms(form.preferences) }, editingId)
  ...
  function edit(profile: DietaryProfile) {
    ...
    setForm({ name: profile.name, allergies: profile.allergies.join(', '), intolerances: profile.intolerances.join(', '), preferences: profile.preferences.join(', ') })
  }
